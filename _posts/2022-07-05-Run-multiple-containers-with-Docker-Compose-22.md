...
Docker Compose 透過 docker-compose.yml 和 `docker-compose` 指令，加上對 Dockerfile 的支援，快速將多個 Docker Container 分別建立並串接在同一個網路下進行整合，並用簡便的指令來管理同一個 docker-compose.yml 所啟動的 Docker Container。  

## 二、什麼情況會需要用到 Docker Compose？
如果所使用的應用系統會需要連結或整合一個以上的 Container 時，就可以考慮使用 Docker Compose 來協助部屬。  

Docker Compose 可用於開發、測試環境或正式運行的生產環境，使用者透過 `docker compose` 指令從 docker-compose.yml 取得設定資訊，就可將應用系統的執行環境和相關設定一併建立完成。 

也可以利用 Dockerfile 搭配 docker-compose.yml 使用，或將 `docker run` 的選項和設定放進 docker-compose.yml 達到更彈性和自動化的部屬。  
...
不過由 docker-compose.yml 啟動的 Container 都可以透過 `docker-compose` 指令和 service 名稱進行操作，因此 Container 的名稱就不用非要指定。  
...
> 用法跟 docker run 和 Dockerfile 的 FROM 指令相同。
{: .prompt-info }
...
### 7. entrypoint
**用途：覆蟹掉 Image 檔案內原有的 ENTRYPOINT 指令。**

> 用法跟 Dockerfile 的 ENTRYPOINT 指令類似。
{: .prompt-info }
...
### 8. environment
**用途：用以設定該 service 的環境變數。**

> 用法跟 docker run -e 或是 Dockerfile 的 ENV 指令類似。
{: .prompt-info }
...
### 9. env_file
**用途：將變數檔案路徑及檔案名稱事先寫在文字檔，並直接引用到 docker-compose.yml 內，可以省去用 environment 指令逐一設定的麻煩。**
...
### 10. expose
**用途：用以設定該 Container/Service 所要啟用的連結埠號。**
...
services:
  http:
    ...
    volumes:
      ...
      # 將 docker volume 指令建立的資料卷掛載到 Container 上
      - nginx-data:/usr/share/nginx/html
```

#### (2) 與 services 指令同層的 volumes （頂層）：建立一組新的資料卷
用頂層的 volumes 指令建立一個名為 `nginx-data` 的資料卷後，就可以使用 services 下的 volumes 將命名好的資料卷進行掛載。  
...
> 可以使用頂層的 networks 定義多個 Docker Network，且每一個 services 下的 Container 也可以透過 networks 指令同時加入多個 Docker Network。
{: .prompt-info }
...
只要在 node-red 服務下方加上下面的 depends_on，就可以讓 node_red 在 db 啟動完成後再啟動。

> 注意！啟動完成是指 Container 本身，不是 Container 內的程式或服務啟動完成。
{: .prompt-warning }

下面例子可能會發生 node-red 在 db 啟動完成後要啟動，但 Couchdb Server 還沒有啟動完成而無法連線。
...
這兩個指令大部分已經可以透過 Docker Nerwork 功能自動達成，建議改用 networks 指令建立 Docker Network 達成相關功能。
...
用 `docker-compose` 指令操作的好處在於可以對指定的 docker-compose.yml 檔案內所啟動的 Container 們做整批的操作，而不需要一個一個處理。
...
2. build
用途：如果 docker-compose.yml 檔案裡有使用到 Dockerfile 來產生 Docker Image 的話，可以使用 build 來讓這些 Dockerfile 產生所需的 Docker Image，但如果docker-compose.yml 檔案裡沒有指定 Dockerfile，用 build 舊部會有任何產出
...
用途：啟動、停止、重新啟動、暫停、取消暫停、強制停止 docker-compose.yml 檔案內的 Container          
...
6. run  用途：主要是利用 services 指令所設定好的各項設定值，然後搭配要執行的命令來啟動服務。         
$ docker-compose run      # node-red 是服務名稱不是 Image 名稱   
選項有：「-d」、「--name」、「--rm」、「-p」、「--entrypoint」、「--workdir」、「-v」、「--no-deps」         
--no-deps：啟動服務時，不去啟動相連或相依的服務          
            
            
7. up 用途：（最常用的指令）將撰寫好的 docker-compose.yml 建立，並啟動所有的服務         
```bash
# -d：在背景模式執行，如果沒有加上 -d，Container 就會在前景執行          
$ docker-compose up -d          
$ docker-compose -p 專案名稱 up  -d         
...
8. version  用途：查看 docker-compose 版本的方式          
...
$ docker-compose ps         
$ docker-compose ps -q    # 只顯示 Container ID 資訊     
            
12. help
用途：查訊個別指令選項的使用方式和參數         
$ docker-compose start --help         
$ docker-compose start -?   # docker 指令不支援這個操作      



## 九、何時要使用 Docker Compose 指令？
docker-compose 指令和 docker 指令相同，所以很常會搞不清楚要用那個指令

經過上面可知，docker-compose 指令操作都是基於 docker-compose.yml，如果想要操作的 Container 是由某一個 docker-compose.yml 檔案所定義和產生時，就用 docker-compose 指令來操作，可以少打一些字，因為 docker-compose 會自動去參考 docker-compose.yml 的相關設定
